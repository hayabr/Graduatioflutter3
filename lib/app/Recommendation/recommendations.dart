...
// Class to store MACD points for stocks
...
class _StockRecommendationPageState extends State<StockRecommendationPage> with RouteAware {
  ...
  Future<void> _fetchStockData() async {
    ...
    for (var stock in stocks) {
      try {
        final data = await _fetchStockDataForSymbol(stock["symbol"]!);
        if (data != null) {
          final recommendation = _generateStockRecommendation(
            stock["symbol"]!,
            stock["name"]!,
            data,
          );
  ...
  List<StockMACDPoint> _calculateMACDHistory(List<double> prices) {
    ...
    List<StockMACDPoint> macdPoints = [];
  ...
  StockRecommendation _generateStockRecommendation(
    ...
    if (closes.length < 26 || highs.length < 15 || lows.length < 15) {
      return StockRecommendation(
        ...
        analysis: ["⚠️ Insufficient data to analyze the stock"],
        conditions: [],
        buySignals: 0,
        sellSignals: 0,
      );
    ...
    String macdCondition = "Weak Sell - MACD indicates a bearish trend"; // Default
    if (isCrossOver) {
      if (isBullish) {
      ...
      macdCondition = "Weak Buy - MACD indicates a bullish trend";
    } else if (latestMACD != null && latestMACD.macdLine < latestMACD.signalLine) {
      macdAnalysis.add("Bearish trend: MACD line is below signal line, indicating bearish momentum (weak sell signal).");
      macdCondition = "Weak Sell - MACD indicates a bearish trend";
    }
    ...
    final conditions = [
      ...
      lastClose < lastSMA * (1 - smaThreshold)
          ? "Strong Buy - Price is below the moving average by ${(smaThreshold * 100).toStringAsFixed(2)}%"
          : lastClose < lastSMA
              ? "Weak Buy - Price is slightly below the moving average"
              : lastClose > lastSMA * (1 + smaThreshold)
                  ? "Strong Sell - Price is above the moving average by ${(smaThreshold * 100).toStringAsFixed(2)}%"
                  : "Weak Sell - Price is slightly above the moving average",

      // 2. RSI
      rsi < 30
          ? "Strong Buy - RSI in oversold territory (<30)"
          : rsi < 50
              ? "Weak Buy - RSI indicates a buying tendency"
              : rsi > 70
                  ? "Strong Sell - RSI in overbought territory (>70)"
                  : "Weak Sell - RSI indicates a selling tendency",

      // 3. Volume
      lastVolume > avgVolume * 1.3 && lastClose > lastSMA
          ? "Strong Buy - High trading volume with price increase"
          : lastVolume > avgVolume && lastClose > lastSMA
              ? "Weak Buy - Slightly high trading volume with price increase"
              : lastVolume > avgVolume * 1.3 && lastClose < lastSMA
                  ? "Strong Sell - High trading volume with price decrease"
                  : lastVolume > avgVolume && lastClose < lastSMA
                      ? "Weak Sell - Slightly high trading volume with price decrease"
                      : lastClose > lastSMA
                          ? "Weak Buy - Price increasing without strong volume"
                          : "Weak Sell - Price decreasing without strong volume",
      ...
      percentChange < -percentChangeThreshold
          ...
          : percentChange < 0
              ...
              : percentChange > percentChangeThreshold
                  ? "Strong Sell - Strong increase (>${percentChangeThreshold.toStringAsFixed(2)}%)"
                  : "Weak Sell - Slight increase",
    ...
    final analysis = [
      if (lastClose > lastSMA * (1 + smaThreshold))
        "• Price is above the moving average by ${(smaThreshold * 100).toStringAsFixed(2)}% (strong sell signal)"
      else if (lastClose > lastSMA)
        "• Price is slightly above the moving average (weak sell signal)"
      else if (lastClose < lastSMA * (1 - smaThreshold))
        "• Price is below the moving average by ${(smaThreshold * 100).toStringAsFixed(2)}% (strong buy signal)"
      else
        "• Price is slightly below the moving average (weak buy signal)",
      if (rsi > 70)
        "• RSI in overbought territory (overbought)"
      else if (rsi > 50)
        "• RSI indicates a selling tendency"
      else if (rsi < 30)
        "• RSI in oversold territory (oversold)"
      else
        "• RSI indicates a buying tendency",
      ...
      if (lastVolume > avgVolume * 1.3)
        "• Trading volume is 30% above average (notable activity)"
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Icon(
                    stock.recommendation.contains('Buy')
                        ? Icons.trending_up
                        : Icons.trending_down,
              ...
              Row(
                ...
                children: [
                  ...
                  Text(
                    'Change: ${stock.changePercent.toStringAsFixed(2)}%',
                    style: TextStyle(
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Average: \$${stock.sma.toStringAsFixed(2)}'),
              ...
              Row(
                ...
                children: [
                  _buildSignalChip('Buy Signals', stock.buySignals, Colors.green),
                  _buildSignalChip('Sell Signals', stock.sellSignals, Colors.red),
                ],
              ),
              const SizedBox(height: 8),
              Text(
