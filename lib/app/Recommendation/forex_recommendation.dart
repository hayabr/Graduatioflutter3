...
class _ForexRecommendationPageState extends State<ForexRecommendationPage> with RouteAware {
  ...
  Future<void> _fetchForexData() async {
    ...
    for (var pair in pairs) {
      try {
        ...
        print('Error fetching data for ${pair["symbol"]}: $e');
      }
  ...
  ForexRecommendation _generateForexRecommendation(
    ...
    final analysis = [
      ...
      "• مستوى الدعم الحالي: ${support.toStringAsFixed(4)}",
      "• مستوى المقاومة الحالي: ${resistance.toStringAsFixed(4)}",
      "• متوسط النطاق الحقيقي (ATR): ${atr.toStringAsFixed(4)}",
    ];
          ...
          Expanded(
            child: _loadingForex
                ...
                : forexRecommendations.isEmpty
                    ? const Center(
                        child: Text('فشل جلب البيانات. تحقق من الاتصال وحاول مجددًا.'))
                    : ListView.builder(
